...
import io.github.mvillafuertem.mapflablup.{JsonFlatten, JsonParser}
...
object ScalciteApplication extends App {
  ...
  override def main(args: Array[String]): Unit = {
    ...
    val re3 = JsonQuery(connection).sql(Seq("SELECT `personalinfo.company.name` AS `pepe.company.name` FROM person", "SELECT `personalinfo.email` AS `pepe.info.email` FROM person").toList)
