...
object RelationalCalciteRepository {
  ...
  val live: ZLayer[Has[String], Nothing, CalciteRepo] = ZLayer.fromFunction(databaseName => new CalciteRepository {

    override def queryForMap(map: collection.Map[String, Any],
                             query: String): stream.Stream[ScalciteError, collection.Map[String, Any]] =
      ZStream.fromEffect {
        Task.effect(NamedDB(Symbol(databaseName.get)) autoCommit { implicit session =>
          session.connection
