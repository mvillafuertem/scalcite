...
import io.github.mvillafuertem.scalcite.flattener.core.Flattener
...
object ScalciteFlattener {
  ...
  implicit val codec: JsonValueCodec[Json] = new JsonValueCodec[Json] {
    ...
    private def decodeString(in: JsonReader): JString = {
      in.rollbackToken()
      JString(in.readString(null))
    }

    private def decodeBoolean(in: JsonReader): Json = {
      in.rollbackToken()
      if (in.readBoolean()) Json.True
      else Json.False
    }

    private def decodeNumber(in: JsonReader, n: Byte): JNumber =
      JNumber({
        in.rollbackToken()
    ...
    private def decodeArray(in: JsonReader, default: Json): JArray =
      JArray(
      ...
      JObject(
        if (in.isNextToken('}')) JsonObject.empty
          ...
          do {
            ...
            if (k.isEmpty) {
              x.put(key, decodeValueRecursive(key.concat("."), in, default))
            } else {
