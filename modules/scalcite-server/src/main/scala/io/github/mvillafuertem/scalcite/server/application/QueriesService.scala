...
import io.github.mvillafuertem.scalcite.server.domain.error.ScalciteError.{ DuplicatedEntity, Unknown }
import io.github.mvillafuertem.scalcite.server.domain.model.Query
import io.github.mvillafuertem.scalcite.server.domain.repository.{ ErrorsRepository, QueriesRepository }
...
final class QueriesService(repository: QueriesRepository[QueryDBO], errorsRepository: ErrorsRepository[ErrorDBO]) extends QueriesApplication {
  override def create(query: Query): stream.Stream[ScalciteError, Query] =
    ...
    } yield stream).mapError { case e: SQLException =>
      e.getSQLState match {
