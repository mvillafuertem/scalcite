package io.github.mvillafuertem.scalcite.server.api.documentation

import akka.http.scaladsl.model
import io.github.mvillafuertem.scalcite.server.domain.error
import io.github.mvillafuertem.scalcite.server.domain.error.ScalciteError
import io.github.mvillafuertem.scalcite.server.domain.error.ScalciteError.DuplicatedEntity
...
//    statusMapping(StatusCode.BadRequest, jsonBody[error.ScalciteError.MessageParsingError.type].example(MessageParsingError).description(badRequestDescription))
  ...
  private[api] lazy val statusConflict: EndpointOutput.StatusMapping[ScalciteError.DuplicatedEntity] =
    statusMapping(StatusCode.Conflict, anyJsonBody[ScalciteError.DuplicatedEntity].example(DuplicatedEntity()).description(conflictDescription))
