...
import io.github.mvillafuertem.scalcite.server.domain.QueriesApplication
...
final class QueriesApi(app: QueriesApplication)(implicit materializer: Materializer) extends ApiJsonCodec with ApiErrorMapping with BootstrapRuntime {
  ...
  lazy val queriesPostRoute: Route = AkkaHttpServerInterpreter
    .toRoute(ScalciteEndpoint.queriesPostEndpoint)(query => buildScalciteResponse(app.create(query).map(_.asJson.noSpaces)))

  lazy val queriesGetRoute: Route = AkkaHttpServerInterpreter
    .toRoute(ScalciteEndpoint.queriesGetEndpoint)(uuid => buildResponse(app.findByUUID(uuid).map(_.asJson.noSpaces)))

  lazy val queriesGetAllRoute: Route = AkkaHttpServerInterpreter
    .toRoute(ScalciteEndpoint.queriesGetAllEndpoint)(_ => buildResponse(app.findAll().map(_.asJson.noSpaces)))
